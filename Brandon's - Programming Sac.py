#BRANDON NGUYEN CLASS ATTENDENCE REPORT:
...
ID = 1            #This would allow the number in the attendence to go up after another line.
...
#This would then collect the information collected from the questions/attendence and print out what and the Warning for the lowest attendence.
print("\nResults:")

#Observes the percentage if its less than 50% or else.
for name, percentages in student_ID.items():
    percentages = int(percentages)
    if percentages <= 50:
        print(f"Student Name : {name}, Stundets Attendence Report : ({percentages}%) - Warning Low Attendence")
    else:
        print(f"Student Name : {name}, Stundets Attendence Report : ({percentages}%)")
#END
